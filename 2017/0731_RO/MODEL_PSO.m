  ...
  for i=1:PSO.swarm_size
      ...
      %gbest
        if swarm(i).rmse<PSOgBest.Distance
            ...
            for N=1:NumberOfOUTPUT
                PSOgBest.RLSE(N).Theata=swarm(i).RLSE(N).iteration(NumberOfTrainPoint).theata;
            end
        end
...
%% plot all result              
        ...
        RMSESP1=sqrt(sum(real(PSOgBest.Error(:,2)).^2)/(NumberOfTrainPoint));
        RMSEDJIA1=sqrt(sum(imag(PSOgBest.Error(:,2)).^2)/(NumberOfTrainPoint));
