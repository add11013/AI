...
NumberOfCons=length(subclust(TotalINPUT,0.007));
      ...
      %gbest
        ...
        if JumpCount>10
            ...
            for ii=1:NumberOfPremiseParameters/4
                swarm(i).Position(j1)=ROgBest.Position(j1)+randn*4*stepsize; %center
                swarm(i).Position(j1+1)=ROgBest.Position(j1+1)+randn*4*stepsize; %std
...
%% plot all result              
        ...
        figure(1)
        output=OriginalData(31:291,1)+real(FinalyHead(:,1));
        plot(x,output);
        ...
        figure(2)
        output=OriginalData(31:291,2)+imag(FinalyHead(:,1));
        plot(x,output);
        ...
        figure(3)
        output=OriginalData(31:291,3)+real(FinalyHead(:,2));
        plot(x,output);
        ...
        figure(4)
        output=OriginalData(31:291,4)+imag(FinalyHead(:,2));
        plot(x,output);
        legend('Target (DJIA)','Forecast (DJIA)');
        xlabel('Trading date index');
        ylabel('Stock price (DJIA)');
        
        RMSEIBM1=sqrt(sum(real(ROgBest.Error(:,1)).^2)/(NumberOfTrainPoint));
        RMSETSMC1=sqrt(sum(imag(ROgBest.Error(:,1)).^2)/(NumberOfTrainPoint));
        RMSEIBM2=sqrt(sum(real(testError(:,1)).^2)/(NumberOfTestPoint));
        RMSETSMC2=sqrt(sum(imag(testError(:,1)).^2)/(NumberOfTestPoint));
        
        RMSESP1=sqrt(sum(real(ROgBest.Error(:,2)).^2)/(NumberOfTrainPoint));
        RMSEDJIA1=sqrt(sum(imag(ROgBest.Error(:,2)).^2)/(NumberOfTrainPoint));
        RMSESP2=sqrt(sum(real(testError(:,2)).^2)/(NumberOfTestPoint));
        RMSEDJIA2=sqrt(sum(imag(testError(:,2)).^2)/(NumberOfTestPoint));
