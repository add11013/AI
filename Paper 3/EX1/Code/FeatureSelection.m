...
for t=1:NumberOfTarget
  ...
  %use function "calculateIIM" to calculate all IIM for feature selection
%      for i=1:NumberOfTarget
%          IIM(i).value=CalculateIIM(llMData(i).value(1:NumberOfTrainPoint,:));
%          i
%      end
        load(['IIM_' ExperimentationName]);
%       save(['IIM_' ExperimentationName]);
    
%% calculate selection gain
...
%% calculate Omega，Omega為每個SP裡的特徵集合(若有重複記一個就好)
    %將第一個SP裡的特徵全部加入Omega
    Omega=SP(1).value;
    ...
    %% calculate w，w is covering rate 
    for i=1:length(NOL)
        w(i)=NOL(i)/NumberOfTarget;
    end
    wMean=mean(w);

    %% calculate gsum，gsum is sum of the feature's selection gain in each selected feature pool
    ...
    %% calculate p，p is contribution index which is covering rate by gsum (W*gsum)
    NumberOfFP=0;
