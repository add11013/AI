...
line([NumberOfTrainPoint NumberOfTrainPoint],[min(real(ABCOgBest.Error(:,1)))*1.5 max(real(ABCOgBest.Error(:,1)))*1.5]);
        ...
        legend('Target (BVSP)','Forecast (BVSP)');
        xlabel('Trading date index');
        ylabel('Stock price (BVSP)');
        ...
        legend('Target (Nikkei 225)','Forecast (Nikkei 225)');
        xlabel('Trading date index');
        ylabel('Stock price (Nikkei 225)');
        ...
        legend('Target (SSEC)','Forecast (SSEC)');
        xlabel('Trading date index');
        ylabel('Stock price (SSEC)');
