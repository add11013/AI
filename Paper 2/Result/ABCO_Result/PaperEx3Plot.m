...
temp=[temp; temp1];
plot(xx,temp)
line([NumberOfTrainPoint NumberOfTrainPoint],[min(real(ABCOgBest.Error(:,1)))*5 max(real(ABCOgBest.Error(:,1)))*5]);
        ...
        legend('Target (DELL)','Forecast (DELL)');
        xlabel('Trading date index');
        ylabel('Stock price (DELL)');
        ...
        legend('Target (Microsoft)','Forecast (Microsoft)');
        xlabel('Trading date index');
        ylabel('Stock price (Microsoft)');
