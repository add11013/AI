...
temp=real(ABCOgBest.Error(:,1));
temp=temp/4;
temp1=real(testError(:,1));
temp1=temp1/4;
temp=[temp; temp1];
plot(xx,temp)
...
x=linspace(1,length(OriginalData)-31,length(OriginalData)-31);
        ...
        figure(1)
        output=OriginalData(31:length(OriginalData)-1,1)+imag(FinalyHead(:,1));
        plot(x,output,'--');
        ...
        figure(2)
        output=OriginalData(31:length(OriginalData)-1,2)+real(FinalyHead(:,1));
        plot(x,output,'--');
        line([NumberOfTrainPoint NumberOfTrainPoint],[min(OriginalData(:,2))*0.5 max(OriginalData(:,2))*1.3]);
        ...
        plot(x,output,'--');
        line([NumberOfTrainPoint NumberOfTrainPoint],[min(OriginalData(:,3))*0.5 max(OriginalData(:,3))*1.3]);
        ...
        plot(x,output,'--');
        line([NumberOfTrainPoint NumberOfTrainPoint],[min(OriginalData(:,4))*0.5 max(OriginalData(:,4))*1.3]);
