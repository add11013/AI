...
for i=1:ABCO.swarm_size
    ...
    for M=1:NumberOfINPUT
        ...
        for ii=1:length(h(M).center)
            ...
            bee(i).Position(j1+3)=randn; %lambda2
            j1=j1+4;
        end
    end
    bee(i).limit=0;
    ...
    for i=1:ABCO.swarm_size
        ...
        for M=1:NumberOfINPUT
            for number=1:length(h(M).center)
                ...
                Lambda2Set.INPUT(M).fuzzyset(number)=bee(i).Position(j1+3);
                j1=j1+4;
            end
        end
        ...
        for ii=1:ABCO.OnlookerBee_size
            ...
            for M=1:NumberOfINPUT
                for number=1:length(h(M).center)
                    ...
                    Lambda2Set.INPUT(M).fuzzyset(number)=OnlookerBee(ii).Position(j1+3);
                    j1=j1+4;
                end
            end
