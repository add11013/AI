          ...
          %將粒子的位置，儲存成termSet，以便後續建造前艦部的fuzzy set
            ...
            for M=1:NumberOfINPUT
                for number=1:length(h(M).center)
                    termSet.INPUT(M).fuzzyset(number).value=[ABCOgBest.Position(j1) ABCOgBest.Position(j1+1)];
                    Lambda1Set.INPUT(M).fuzzyset(number)=ABCOgBest.Position(j1+2);
                    Lambda2Set.INPUT(M).fuzzyset(number)=ABCOgBest.Position(j1+3);
                    j1=j1+4;
                end
            end
...
%                     testBeta(3).value(rule,jj)=membership3;
        ...
        %new_yHead(output)
        for jj=1:NumberOfTestPoint
             testyHead(jj,:)=testA(jj).value*ABCOgBest.theta;  %y
        end
        %caculate error    
       for N=1:NumberOfOUTPUT
